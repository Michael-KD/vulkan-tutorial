...
//const std::string MODEL_PATH = "models/low_poly_person.obj";
const std::string TEXTURE_PATH = "textures/viking_room.png";
//const std::string TEXTURE_PATH = "textures/tex_char_new.png";
...
private:
	...
	void mainLoop() {
		...
		while (!glfwWindowShouldClose(window)) {
			...
			if (elapsed.count() >= 1.0f) {
				...
				std::string title = "Vulkan - FPS: " + std::to_string(fps);
	...
	// VERTEX BUFFER AND INDEX BUFFER FUNCTIONS
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		ubo.model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, sin(time), 0.0f)) * glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	//	ubo.view = glm::lookAt(glm::vec3(2.0f * zoom, cameraHeight, 2.0f * zoom), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	// RENDERING AND PRESENTATION FUNCTIONS
	void drawFrame() {
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		for (const auto& availableFormat : availableFormats) {
			if (availableFormat.format == VK_FORMAT_B8G8R8A8_SRGB && availableFormat.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
				//std::cout << "Surface Format Selected: B8G8R8A8_SRGB" << std::endl;
